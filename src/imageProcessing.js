...
exports.genPalette = exports.quantizeAndDisplay = exports.displayImageFromArray = exports.getDataFromElement = exports.getImageData = exports.height = exports.width = exports.KMeansCalc = exports.imageColorData = exports.KMeansCalculator = exports.Color = void 0;
class Color {
    constructor(r, g, b, a) {
        r = this.clampByte(r);
        g = this.clampByte(g);
...
exports.KMeansCalculator = KMeansCalculator;
exports.imageColorData = [];
exports.width = 0;
exports.height = 0;
function getImageData(imageElement) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        exports.width = imageElement.naturalWidth;
        exports.height = imageElement.naturalHeight;
        canvas.width = exports.width;
        canvas.height = exports.height;
        console.log("size: " + exports.width + ", " + exports.height);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let y = 0; y < exports.height; y++) {
            for (let x = 0; x < exports.width; x++) {
                let index = (y * exports.width + x) * 4; // each pixel is 4 array elements (R, G, B, A)
                let red = imageData.data[index];
                let green = imageData.data[index + 1];
                let blue = imageData.data[index + 2];
                let alpha = imageData.data[index + 3];
                let color = new Color(red, green, blue, alpha);
                exports.imageColorData.push(color);
            }
        }
        // sample data less often for images larger than 1000x500 pixels
        let iX = 1;
        let iY = 1;
        if (fast && exports.width >= 1000) {
            // round or floor? does it matter?
            iX = Math.round(exports.width / 1000);
        }
        if (fast && exports.height >= 500) {
            // round or floor? does it matter?
            iY = Math.round(exports.height / 500);
        }
        console.log("halfway done processing data");
        let paletteProcessingColorData = [];
        for (let y = 0; y < exports.height; y += iY) {
            for (let x = 0; x < exports.width; x += iX) {
                let index = y * exports.width + x;
                // #FIXME always 127 alpha cutoff?
                if (exports.imageColorData[index].A < 127)
                    continue;
                paletteProcessingColorData.push(exports.imageColorData[index]);
            }
        }
        ...
        exports.KMeansCalc = new KMeansCalculator(paletteProcessingColorData, 127);
    });
}
...
exports.displayImageFromArray = displayImageFromArray;
...
function quantizeAndDisplay(outputImgElement, debugLabel, strPalette, _width, _height) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // #FIXME: always 127 alpha cutoff?
        let temp = "";
        let lineSize = exports.width * 100;
        ...
        for (let _y = 0; _y < _height; _y++) {
            for (let _x = 0; _x < _width; _x++) {
                let y = Math.floor(_y * (exports.height / _height));
                let x = Math.floor(_x * (exports.width / _width));
                let inputDataIndex = y * exports.width + x;
                let outputDataIndex = _y * _width + _x;
                if (exports.imageColorData[inputDataIndex].A < 127) {
                    outputImageColorData.push(colorWhite); // #FIXME: use black for transparency? white?
                    temp += "0";
                }
                else {
                    let nearIndex = exports.imageColorData[inputDataIndex].nearestInPaletteIndex(palette);
...
exports.quantizeAndDisplay = quantizeAndDisplay;
function genPalette(k, maxIterations, debugLabel, nonGenPalette) {
    return __awaiter(this, void 0, void 0, function* () {
        let startTime = new Date().getTime();
        let palette = yield exports.KMeansCalc.calculateKMeans(k, maxIterations, debugLabel);
